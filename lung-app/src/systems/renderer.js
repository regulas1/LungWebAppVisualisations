...
zincRenderer = new Zinc.Renderer( {antialias: false });
zincRenderer.getThreeJSRenderer().setClearColor(0x000000, 1);
zincRenderer.getThreeJSRenderer().setPixelRatio(1.0);
zincRenderer.addPreRenderCallbackFunction(updateFrame(state.renderer));
el.appendChild(zincRenderer.domElement); // Add to dom tree to make canvas element visible
      ...
      if (!WEBGL.isWebGLAvailable()) {
          ...
          zincRenderer = new Zinc.Renderer(document.getElementById('app'), window); //change id to app
      ...
      // Call from component with all data
